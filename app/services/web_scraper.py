...
class WebScraper:
    ...
    def fetch_sitemap_urls(base_url):
        try:
            ...
            try:
                if not WebScraper.is_safe_url(b + '/robots.txt'):
                    return set()
            ...
            for sm in candidates:
                try:
                    if not WebScraper.is_safe_url(sm):
                        continue
    ...
    def fetch_one_page_requests(url):
        try:
            ...
            if not WebScraper.is_safe_url(url):
                return False, None, 'Unauthorized (SSRF Blocked)'
    ...
    def fetch_targeted_pages(url, question, max_pages=15):
        ...
        try:
            ...
            try:
                def _task(u):
                    if not WebScraper.is_safe_url(u):
                        return u, False, None, "SSRF Blocked"
            ...
            if suspicious_high_value:
                try:
                    ...
                    with pwa.sync_playwright() as p:
                        ...
                        context = browser.new_context(ignore_https_errors=False, user_agent='Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/115.0.0.0 Safari/537.36')
                        page = context.new_page()
