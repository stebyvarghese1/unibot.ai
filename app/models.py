...
class User(db.Model):
    ...
    show_tour = db.Column(db.Boolean, default=True)
    is_active = db.Column(db.Boolean, default=True)

    # Relationships
    sessions = db.relationship('ChatSession', backref='user', cascade="all, delete-orphan", primaryjoin="User.id == ChatSession.user_id")
    messages = db.relationship('ChatMessage', backref='user', cascade="all, delete-orphan", primaryjoin="User.id == ChatMessage.user_id")
    documents = db.relationship('Document', backref='user', cascade="all, delete-orphan", primaryjoin="User.id == Document.uploaded_by")
...
class Document(db.Model):
    ...
    doc_type = db.Column(db.String(50), default='syllabus', nullable=True) # syllabus, system_info, general
    structure_json = db.Column(db.Text, nullable=True) # Stores the extracted Units/Modules and Topics
    
    chunks = db.relationship('DocumentChunk', backref='document', lazy='subquery', cascade="all, delete-orphan")
...
class ChatMessage(db.Model):
    ...
    feedback = db.Column(db.String(20), nullable=True) # 'like', 'dislike', None
    created_at = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))
...
class ChatSession(db.Model):
    ...
    messages = db.relationship('ChatMessage', backref='session', lazy='selectin', cascade="all, delete-orphan",
...
class AppSetting(db.Model):
    ...
    def get(key, default=None):
        ...
        if key in AppSetting._cache and (now - AppSetting._cache_time.get(key, 0)) < AppSetting.CACHE_TTL:
            return AppSetting._cache[key]
            
        row = AppSetting.query.filter_by(key=key).first()
        val = row.value if row else default
        
        AppSetting._cache[key] = val
        AppSetting._cache_time[key] = now
        return val
    ...
    def set(key, value):
        ...
        # Invalidate cache
        AppSetting._cache[key] = value
        AppSetting._cache_time[key] = datetime.now(timezone.utc).timestamp()
