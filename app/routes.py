...
# Removed redundant local scraping helpers

# Removed redundant local scraping logic in favor of app/services/web_scraper.py
        for u in cands:
    ...
    try:
        ...
        if urls_raw:
            try:
                ...
                urls = []
        if not urls and url:
            urls = [(WebScraper.normalize_url(url) or '').strip()]
        ...
        if isinstance(urls, list):
            ...
            for u in urls:
                if not isinstance(u, str):
                    continue
                u2 = (WebScraper.normalize_url(u) or '').strip()
...
def query():
    try:
        ...
        # --- Pre-FLight Security: Prompt Injection Check ---
        if AIService.is_injection_attempt(question):
            logging.warning(f"Blocked prompt injection attempt from user {uid}: {question}")
            return jsonify({
                'answer': "I'm sorry, but I cannot fulfill this request as it contains instructions that potentially conflict with my safety or operational policies. How else can I help you?",
        ...
        try:
            ...
            if mode == 'general':
                ...
                # Generate
                answer = AIService.generate_answer_from_website(question, context, source_url=target_urls[0], history=history)
