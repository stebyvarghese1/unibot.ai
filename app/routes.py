from flask import Blueprint, request, jsonify, render_template, session, current_app, redirect, Response, stream_with_context
    ...
    if file and allowed_file(file.filename):
        ...
        try:
            ...
            new_doc = Document(
                ...
                status='pending',
                course=course if doc_type == 'syllabus' else None,
                semester=semester if doc_type == 'syllabus' else None,
                subject=subject if doc_type == 'syllabus' else None,
                doc_type=doc_type
            )
...
def query():
    try:
        ...
        if AIService.is_smalltalk(question):
            ...
            msg = ChatMessage(user_id=uid, question=question, answer=answer, sources_json='[]', session_id=session_id)
            db.session.add(msg)
            db.session.commit()
            return jsonify({'answer': answer, 'sources': [], 'session_id': session_id, 'session_title': session_title})
        ...
        if mode == 'general':
            ...
            target_urls = json.loads(urls_raw) if urls_raw else ([primary_url] if primary_url else [])
            ...
            for url in target_urls:
                ...
                if ok and index: all_index.extend(index)
            
            if not all_index:
                return jsonify({'answer': 'I could not retrieve any content from the configured website(s).', 'sources': []})
            ...
            q_vec = q_embedding[0] if isinstance(q_embedding, list) and q_embedding else None
            
            if not q_vec:
                return jsonify({'error': 'Failed to embed question'}), 500
                
            id_keywords = ['who are you', 'what are you', 'your name', 'created you', 'unibot']
            ...
            for r in results:
                ...
                if not dtype or dtype == 'syllabus':
                    if did and did in doc_map: dtype = getattr(doc_map[did], 'doc_type', 'syllabus')
            ...
            if not pre_filtered and results: pre_filtered = results[:3]
            ...
            if course or semester or subject:
                def match_cat(r):
                    ...
                    if not dtype or dtype == 'syllabus':
                        if did and did in doc_map: dtype = getattr(doc_map[did], 'doc_type', 'syllabus')
                    if dtype == 'system_info': return True
                    d = doc_map.get(did)
                    if not d: return False
                    return (not course or d.course == course) and (not semester or d.semester == semester) and (not subject or d.subject == subject)
                filtered = [r for r in filtered if match_cat(r)]

            system_bits = [r for r in filtered if (r.get('doc_type') == 'system_info' or (r.get('doc_id') and doc_map.get(r.get('doc_id')).doc_type == 'system_info'))]
            academic_bits = [r for r in filtered if r not in system_bits]
            
            if identity_intent:
                sys_limit, acad_limit = 8, 2
            else:
                sys_limit = 1 if system_bits and system_bits[0].get('distance', 10) < Config.VECTOR_MAX_DISTANCE/2 else 0
                acad_limit = 10
            ...
            for r in final_context_bits:
                did = r.get('doc_id') or r.get('document_id')
                dtype = r.get('doc_type') or (doc_map.get(did).doc_type if did in doc_map else None)
                if dtype == 'system_info': continue
                if did not in unique and did in doc_map:
                    unique[did] = {'doc_id': did, 'filename': doc_map[did].filename, 'url': SupabaseService().get_public_url(doc_map[did].file_path)}
            sources = list(unique.values())

        if stream:
            def generate():
                full_answer = ""
                generator = AIService.generate_answer_stream(standalone_question, context, history=history) if mode == 'studies' else AIService.generate_answer_from_website_stream(standalone_question, context, history=history)
                yield f"data: {json.dumps({'meta': {'session_id': session_id, 'session_title': session_title, 'sources': sources}})}\n\n"
                for chunk in generator:
                    full_answer += chunk
                    yield f"data: {json.dumps({'token': chunk})}\n\n"
                with current_app.app_context():
                    db.session.add(ChatMessage(user_id=uid, question=question, answer=full_answer, sources_json=json.dumps(sources), session_id=session_id))
                    db.session.commit()
                yield "data: [DONE]\n\n"
            return Response(stream_with_context(generate()), mimetype='text/event-stream')
        else:
            answer = AIService.generate_answer(standalone_question, context, history=history) if mode == 'studies' else AIService.generate_answer_from_website(standalone_question, context, history=history)
            msg = ChatMessage(user_id=uid, question=question, answer=answer, sources_json=json.dumps(sources), session_id=session_id)
            db.session.add(msg)
            db.session.commit()
            return jsonify({'answer': answer, 'sources': sources, 'session_id': session_id, 'session_title': session_title, 'message_id': msg.id})
